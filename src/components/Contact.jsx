...
export default function Contact() {
  ...
  const verifyFields = (email) => {
    ...
    if (name.length > 0 && regexEmail.test(email) && message.length > 0) {
      setIsDisabled(false)
      return
    }
    setIsDisabled(true)
      ...
      <form className="px-4" noValidate action="https://formsubmit.co/quelvinmp@gmail.com" method="POST">
        ...
        <div className="flex flex-col items-center justify-center w-full gap-8 m-auto">
          ...
          <div className="w-full max-w-2xl form-control">
            ...
            <input type="email" placeholder="Digite aqui" name="email" className="w-full max-w-2xl input input-bordered" value={email} required onChange={({target: {value}}) => {setEmail(value)
            verifyFields(value)}}/>
          </div>
          ...
          <button type="submit" className="btn btn-lg btn-primary"  disabled={isDisabled}>Submit Form</button>
